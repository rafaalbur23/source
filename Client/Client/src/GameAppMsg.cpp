	...
	else if (nFlag == 1) // 保存并释放Section数据
	{
		...
		for(int i = 0; i < pScene->GetSceneObjCnt(); i++)
		{
			...
			if(pObj->IsValid())
			{
				...
				{
//					pEffect->HandleSceneMsg(SCENEMSG_SCENEOBJ_DESTROY, pObj->getEffectID(i),pObj->getID());
					if (pThis->CanEdit())
					{
...
void CGameApp::HandleKeyDown(DWORD dwKey)
{
	...
	else if (IsKeyDown(DIK_K) && IsCtrlPress() && CGameScene::GetMainCha()->getGMLv()) 
	{ 
		CCharacter* pMain = CGameScene::GetMainCha();
		...
		{
			char buf[128] = { 0 };
			sprintf(buf, "Coords :(%d,%d) Angle :%d Pitch : %d Roll %d", pMain->GetCurX(), pMain->GetCurY(), pMain->getYaw(), pMain->getPitch(),pMain->getRoll());
...
void CGameApp::HandleContinueSuperKey()
{
	...
	if(IsKeyContinue(DIK_W))
	{
		...
		{
			CCharacter *pMain = _pCurScene->GetMainCha();
			if(pMain)
			{
				_cMouseDown.ActMove( pMain, pMain->GetCurX(), pMain->GetCurY()-125 );
			}
		}
	...
	if(IsKeyContinue(DIK_A))
	{
		if(g_pGameApp->m_keyMoveMode)
		{
			...
			if(pMain)
			{
				_cMouseDown.ActMove( pMain, pMain->GetCurX()-125, pMain->GetCurY() );
			}
		}
	}
