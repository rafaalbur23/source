...
UIRender::UIRender()
: _nGuiWidth(LARGE_RES_X), _nGuiHeight(LARGE_RES_Y), _fScreenScaleX(1.0), _fScreenScaleY(1.0)
...
void UIRender::SetScreen( int w, int h, bool isFull )
{
	...
	_fDrawScaleX = (float)((float)w / LARGE_RES_XS);
	_fDrawScaleY = (float)((float)h / LARGE_RES_YS);
...
void CGuiFont::SetScreen( int nScrWidth, int nScrHeight )
{
	...
	_stfonts.push_back( ft1 );

	stFont ft2;
	ft2.strFont	= "DEC Terminal Modern";
	ft2.size800	= 13;
	ft2.size1024 = 14;
	ft2.dwStyle	= 0;

	_stfonts.push_back( ft2 );
	...
	else if( nScrWidth==EXTRA_LARGE_RES_X )
	{
		for( stfont::iterator it=_stfonts.begin(); it!=_stfonts.end(); ++it )
		{
			p =  new CMPFont;
	...
	else if( nScrWidth==FULL_LARGE_RES_X )
	{
		for( stfont::iterator it=_stfonts.begin(); it!=_stfonts.end(); ++it )
		{
			p =  new CMPFont;
