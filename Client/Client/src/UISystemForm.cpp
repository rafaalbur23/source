...
bFullScreen(false), bPixel1024(0), nTexture(0), nQuality(0),
...
int CSystemProperties::ApplyVideo()
{
	...
	else if (m_videoProp.bPixel1024 == 1)
	{
		width = MID_RES_X;
		height = MID_RES_Y;
	}
	else if (m_videoProp.bPixel1024 == 2)
	{
		width = LARGE_RES_X;
		height = LARGE_RES_Y;
	}
	else if (m_videoProp.bPixel1024 == 3)
	{
		width = EXTRA_LARGE_RES_X;
		height = EXTRA_LARGE_RES_Y;
	}
	else if (m_videoProp.bPixel1024 == 4)
	{
...
#define DEFAULT_NUM -2
	...
	if (iTemp == DEFAULT_NUM)	return DEFAULT_NUM;
	m_gameOption.bIsShowMount = int2bool(iTemp);
...
#define ERROE_FILE_CANNT_CREAT -2
	...
	if (!WriteInteger("video", "fullScreen", bool2int(m_videoProp.bFullScreen), szIniFileName))
		return OTHER_ERROR;
	if (!WriteInteger("video", "pixel1024", m_videoProp.bPixel1024, szIniFileName))
		return OTHER_ERROR;
	...
	if (!WriteInteger("gameOption", "is60fps", bool2int(m_gameOption.bIs60FPSMode), szIniFileName))
		return OTHER_ERROR;	
	if (!WriteInteger("gameOption", "showmounts", bool2int(m_gameOption.bIsShowMount), szIniFileName))
		return OTHER_ERROR;	
...
void CSystemMgr::LoadCustomProp()
{
	...
	if (!m_isLoad)
	{
		if (m_sysProp.Load("user\\system.ini"))
		{
			...
			m_sysProp.m_videoProp.bPixel1024= 0;//g_Render.GetScrWidth() > 1000 ? true : false;
			...
			m_sysProp.m_gameOption.bIsShowMount = true;
	...
	IsShowMount = m_sysProp.m_gameOption.bIsShowMount;
...
bool CSystemMgr::Init()
{
	...
	*/
	...
	cbxSize = ( CCombo *)frmVideo->Find( "cbxSize" ); //贴图精度
	if (! cbxSize )		return Error( g_oLangRec.GetString(45), frmVideo->GetName(), "cbxSize" );
	cbxSize->GetList()->GetItems()->Select(m_sysProp.m_videoProp.bPixel1024);
	...
	if (!cbxIs60FPSMode) return Error(g_oLangRec.GetString(45), frmGameOption->GetName(), "cbxIs60FPSMode");
	
	cbxIsShowMount = (CCheckGroup*)frmGameOption->Find("cbxIsShowMount");
	if (!cbxIsShowMount) return Error(g_oLangRec.GetString(45), frmGameOption->GetName(), "cbxIsShowMount");
...
void CSystemMgr::End()
{
	const char* Value = cbxSize->GetText();
	int Res;
	
	if (strcmp("1024x576", Value) == 0)
	{
		Res = 0;
	}
	...
	else if(strcmp("1360x765", Value) == 0)
	{
		Res = 2;
	}
	else if(strcmp("1440x810", Value) == 0)
	{
		Res = 3;
	}
	...
	if (cbxSize)
		m_sysProp.m_videoProp.bPixel1024 = Res;
...
void CSystemMgr::_evtVideoFormMouseEvent(CCompent *pSender, int nMsgType, int x, int y, DWORD dwKey)
{
	...
	{			
		...
		const char* Value = g_stUISystem.cbxSize->GetText();	
		if (strcmp("1024x576", Value) == 0)
		{
			width = SMALL_RES_X;
			height = SMALL_RES_Y;
...
void CSystemMgr::_evtGameOptionFormMouseDown(CCompent *pSender, int nMsgType, int x, int y, DWORD dwKey)
{
	...
	if (pGroup)
	{
		...
		if (bCameraMode != g_stUISystem.m_sysProp.m_gameOption.bCameraMode)
		{
			...
			::WritePrivateProfileString("gameOption", "cameraMode", bCameraMode ? "0" : "1", "./user/system.ini");
		}
	}
	...
	pGroup = g_stUISystem.cbxIsShowMount;
	if (pGroup)
	{
		bool bIsShowMount = pGroup->GetActiveIndex() == 1 ? true : false;
		g_stUISystem.m_sysProp.m_gameOption.bIsShowMount = bIsShowMount;
		IsShowMount = bIsShowMount;
		g_pGameApp->SysInfo(IsShowMount ? "Mounts Show Mode On " : "Mounts Show Mode OFF");
		CGameScene* curScene = g_pGameApp->GetCurScene();
		CCharacter* pCha = curScene->_pChaArray;
		for (int i = 0; i < curScene->_nChaCnt; i++) {
		if (pCha->IsValid() && !pCha->IsNPC() && pCha->IsEnabled()) {
	...
	::WritePrivateProfileString("gameOption", "showmounts", bIsShowMount ? "1" : "0", "./user/system.ini");
	}
...
void CSystemMgr::_evtGameOptionFormBeforeShow(CForm* pForm, bool& IsShow)
{
	...
	pGroup = g_stUISystem.cbxIsShowMount;
    if (pGroup)
        pGroup->SetActiveIndex(g_stUISystem.m_sysProp.m_gameOption.bIsShowMount ? 1 : 0);
