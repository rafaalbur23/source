...
{
	DWORD dwAveragePing; // µ¥Î»£ººÁÃë£¬Í¨¹ý NetIF::GetAveragePing() »ñµÃ£¬´ËÖµ³ËÒÔËÙ¶È¾ÍÊÇÔ¤ÒÆ¶¯µÄ¾àÀë
	POINT pos_buf[defMAX_POS_NUM]; // ÒÆ¶¯¹ÕµãÐòÁÐ£¨°üº¬µ±Ç°Î»ÖÃ£©
	DWORD pos_num; // ÓÐÐ§µãµÄ¸öÊý
...
struct stNetNotiMove // enumACTION_MOVE
{
	...
	POINT	SPos[defMAX_POS_NUM];	// ÒÆ¶¯¹ÕµãÐòÁÐ£¨°üº¬µ±Ç°Î»ÖÃ£©
	long	nPointNum;				// ÓÐÐ§µãµÄ¸öÊý
...
struct stEffect
{
	long	lAttrID;	// Ó°ÏìµÄÊôÐÔID
	LONG64	lVal;		// Ó°ÏìµÄÊýÖµ
...
struct stSkillState
{
	...
	stSkillState() : chID(0), chLv(0),lTimeRemaining(0) { };
};
...
struct stNetNotiSkillRepresent //技能表现 enumACTION_SKILL_SRC
{
	...
	CSizeArray<stEffect>		SEffect;	// Ó°ÏìµÄ½á¹ûÐòÁÐ
...
struct stNetNotiSkillEffect // 技能的作用结果 enumACTION_SKILL_TAR
{
	...
	CSizeArray<stEffect>		SSrcEffect;	// Ó°ÏìµÄ½á¹ûÐòÁÐ
...
{
	stNetActorCreate() : ulWorldID(0), isvip(0), lHandle(0), ulChaID(0), chCtrlType(0), sAngle(0), ulTLeaderID(0), sState(0)
...
{
	const char *m_sysinfo;		// ÏµÍ³ÐÅÏ¢ÄÚÈÝ
...
// 
// ×°±¸µÀ¾ß
// 1.ÔÚµÀ¾ßÀ¸Ñ¡ÔñµÀ¾ßÍÏ¶¯µ½×°±¸À¸,·¢ËÍ×°±¸Ð­Òé
//    ³É¹¦:1.Í¨ÖªËùÓÐ¿Í»§¶ËÍâ¹Û·¢Éú±ä»¯,Í¬Ê±Ö÷¿Í»§¶Ë¸üÐÂÏàÓ¦×°±¸À¸
//         2.Í¨ÖªÖ÷¿Í»§¶ËµÀ¾ßÀ¸·¢Éú±ä»¯

struct stNetItemCreate	// ÎïÆ·³öÏÖ£¬ÏûÊ§
{
	unsigned long lWorldID;
	long	lHandle;		// ·þÎñÆ÷ÐÅÏ¢£¬Ô­Öµ·µ»Ø
	long	lEntityAddr;	// ·þÎñ¶ËµÄÊµÌåµØÖ·
	long	lID;
	Point	SPos;
	short	sAngle;
	short	sNum;			// 物品个数
