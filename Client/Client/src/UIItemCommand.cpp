...
const DWORD COLOR_GREEN = 0xFF05f043;

const unsigned int ITEM_HEIGHT = 32;
const unsigned int ITEM_WIDTH = 32;

map<int, DWORD> CItemCommand::_mapCoolDown; // ±£´æÉÏÒ»´Î·ÅµÄµÀ¾ß¼¼ÄÜµÄÊ±¼ä
...
void CItemCommand::SaleRender(int x, int y, int nWidth, int nHeight) {
	...
	if (_ItemData.sID == 15046 || _ItemData.sID == 15047 || _ItemData.sID == 15048 || _ItemData.sID == 15049 || _ItemData.sID == 15050 || _ItemData.sID == 15051 || _ItemData.sID == 15052 || _ItemData.sID == 15053 || _ItemData.sID == 15054 || _ItemData.sID == 15055 || _ItemData.sID == 15056) {
	...
	} else if (sType == 59 && _ItemData.sEndure[1] != 25000) { // render fairy level on icon
		static SItemHint item;
		memset(&item, 0, sizeof(SItemHint));
		item.Convert(_ItemData, _pItem);
...
void CItemCommand::AddHint(int x, int y) {
	...
	bool isEquip = (_pItem->sType == 20 || _pItem->sType == 23 || _pItem->sType == 24);
	...
	if (isWeapon || isDefenceType || isJewelery || isEquip) {
		...
		if (_pItem->lID == 1034) {
			sprintf(buf, g_oLangRec.GetString(862), _ItemData.sEndure[0] * 10 - 1000, _ItemData.sEndure[1] * 10 - 1000); // ¾ßÌå¼ÆËã·½Ê½Ñ¯ÎÊ²ß»®
			PushHint(buf, COLOR_WHITE, 5, 1);
			return;
		}
		...
		if (!isStore) // ÉÌ³ÇÄÚ²»ÏÔÊ¾¾«ÁéµÄÌåÁ¦ºÍ³É³¤¶È
		{
	...
	else if (_pItem->sType == 42) // ÈÎÎñµÀ¾ß
	{
		PushHint(_pItem->szName, COLOR_BLUE, 5, 1);

		SetHintIsCenter(false);
		_AddDescriptor();
	} else if (_pItem->sType == 18 || _pItem->sType == 19) // ¸«Í·,Ìú¸ä
	{
		PushHint(_pItem->szName, COLOR_WHITE, 5, 1);
		...
		if (_ItemData.GetItemLevel() > 0) {
			sprintf(buf, g_oLangRec.GetString(627), _ItemData.GetItemLevel() * 2 + 100); // ·¢»Ó¶È¶¨Òå 0¼¶-80% 1¼¶-82% ...
			PushHint(buf, GENERIC_COLOR);
		}
	...
	} else if (_pItem->sType == 29) // ±´¿Ç
	{
		...
		_AddDescriptor();
	} else if (_pItem->sType == 45) // Ã³Ò×Ö¤Ã÷
	{
		PushHint(_pItem->szName, COLOR_WHITE, 5, 1);
	...
	} else if (_pItem->sType == 46) // ÓÂÕßÖ®Ö¤
	{
		PushHint(_pItem->szName, COLOR_WHITE, 5, 1);
		...
		sprintf(buf, "%s:%d", g_oLangRec.GetString(848), _ItemData.sEnergy[1]); // ÂÒ¶·µãÊý
		PushHint(buf, GENERIC_COLOR);
		...
		PushHint(GetStoneHint(1).c_str()); // µ¥¸ö±¦Ê¯¶¼ÏÔÊ¾1¼¶µÄÊôÐÔ
		_AddDescriptor();
	} else if (_pItem->sType == 50) {
		sprintf(buf, g_oLangRec.GetString(656), ConvertNumToChinese(item.sEnergy[1]).c_str(), _pItem->szName);
		PushHint(buf, COLOR_WHITE, 5, 1);
		_AddDescriptor();
	} else if (_pItem->sType == 65) // ºÚÁú¼ÀÌ³  add by Philip.Wu  2006-06-19
	{
		...
		if (3279 <= _pItem->lID && _pItem->lID <= 3282 || _pItem->lID == 6370 || _pItem->lID == 6371 || _pItem->lID == 6376 ||
			...
			_pItem->lID == 5897 || (_pItem->lID >= 6383 && _pItem->lID <= 6385)) // ¶ÁÊéÏµÍ³ÌØÊâ´¦Àí£¬±©¶ñÐÄ
		{
			PushHint(_pItem->szName, COLOR_WHITE, 5, 1);
		...
		if (_pItem->lID == 2911 || _pItem->lID == 2912 || _pItem->lID == 2952 ||
			_pItem->lID == 3066 || _pItem->lID == 3078) // ÈÎÎñµÀ¾ßÌØÊâ´¦Àí
		{
			int nMonth = 0;
			int nDay = 0;
			int nHour = 0;
			...
			return;
		} else if (_pItem->lID == 2954) // ÌØÊâµÀ¾ß´¦Àí£¨ËÀÍöÖ¤Ã÷£©
		{
			int nCount = 0;
			for (int i = 0; i < 5; ++i) {
			...
			sprintf(buf, "%s: %d", g_oLangRec.GetString(933), nCount); // "ËÀÍö´ÎÊý"
			PushHint(buf);
			...
			return;
		} else if (_pItem->lID == 579) // ÈëÑ§Ö¤Êé µ±×÷ÆÕÍ¨µÀ¾ßÏÔÊ¾ÌØÊâ´¦Àí
		{
			SetHintIsCenter(false);
			_AddDescriptor();
		...
		//Begin
			...
			if (_pItem->nID == 6373) {
				...
				sprintf(buf, "´æ´¢Ê±¼äÎª£º%d", nCount);
			}
		...
		return;
	} else if (_pItem->sType == 69) // XXXÍ¼Ö½
	{
		int iItem = 0;
		long lForge = 0;
		...
		if (_pItem->lID == 2902 || _pItem->lID == 2903) // ¹«Ö÷ÇéÔµ·û, Íõ×ÓÇéÔµ·û
		{
			sprintf(buf, g_oLangRec.GetString(909), item.sInstAttr[ITEMATTR_VAL_STR]); // "ÇéÔµ±àÂë£º%d"
			PushHint(buf, GENERIC_COLOR);
		...
		if (_pItem->lID != 2236) {
			...
			sprintf(buf, g_oLangRec.GetString(897), _ItemData.sEnergy[0]); // "¹¤¾ß¾­Ñé£º%i"
			PushHint(buf, GENERIC_COLOR);
		}
		...
		SetHintIsCenter(true);
		_AddDescriptor();
	} else if (_pItem->sType == 71 && _pItem->lID == 3289) //Ñ§ÉúÖ¤´¦Àí
	{
		//add by ALLEN 2007-10-16

		// Ñ§ÉúÖ¤ÌØÊâ´¦Àí£¨¼«¶È¶ñÐÄ£¡£¡£©
		PushHint(_pItem->szName, COLOR_WHITE, 5, 1); // Ãû×Ö

		int nLevel = item.chForgeLv;
		const char* arShowName[5] = {g_oLangRec.GetString(944), g_oLangRec.GetString(945), g_oLangRec.GetString(946), g_oLangRec.GetString(947), g_oLangRec.GetString(948)}; // "Ó×¶ùÔ°", "Ð¡Ñ§", "³õÖÐ", "¸ßÖÐ", "´óÑ§" };
		if (0 <= nLevel && nLevel <= 4) {
			sprintf(buf, g_oLangRec.GetString(949), arShowName[nLevel]); // "Ñ§Àú:%s"
			PushHint(buf, COLOR_WHITE, 5, 1);
		}

		sprintf(buf, g_oLangRec.GetString(950), item.sEndure[0], item.sEndure[1]); // "Ñ§·Ö(%d/%d)"
		PushHint(buf, COLOR_WHITE, 5, 1);

		sprintf(buf, g_oLangRec.GetString(951), item.sEnergy[0] * 1000, item.sEnergy[1] * 1000); // "¾­Ñé(%d/%d)"
		PushHint(buf, COLOR_WHITE, 5, 1);

		return;
	}
	//	Add by alfred.shi 20080922 begin. ×çÖäÍÞÍÞ´¦Àí¡£
	...
	//	End.
		...
		if (_pItem->lID == 58) // ·èÅ®ÈËÒªÇóÌØÊâ´¦Àí£¬Ð·Ãç
		{
			PushHint(_pItem->szName, COLOR_WHITE, 5, 1);
			...
			return;
		} else if (_pItem->lID == 171) // bragiÒªÇóÌØÊâ´¦Àí
		{
			PushHint(_pItem->szName, COLOR_WHITE, 5, 1);
			...
			return;
		} else if (_pItem->lID == 2967) // ÂíÉ©ÒªÇóÌØÊâ´¦Àí£¬ºì¾Æ
		{
			PushHint(_pItem->szName, COLOR_WHITE, 5, 1);
	...
	if (_hints.GetCount() > 0 && (_pItem->sType <= 27 || _pItem->sType == 44) && _pItem->sType != 18 && _pItem->sType != 19 && _pItem->sType != 21) {
		// ÄÜÁ¿×î´óÖµµÄ°ÙÎ»´ú±íÑÕÉ«,Ê®Î»¼°ÒÔÏÂ´ú±íÃû×ÖÇ°×º
		char szBuf[16] = {0};
		sprintf(szBuf, "%09d", _ItemData.sEnergy[1] / 10); // ³ýÊ®ÊÇÒòÎª·þÎñÆ÷µÄÊýÖµÎª4Î»Êý,µÈ´ýÒÔºó¸ÄÎªÈýÎ»
...
bool _wait_select_lock_item_state_ = false; //	ÓÃ»§ÒÑ¾­Ê¹ÓÃËøµÀ¾ß£¬ÕýÔÚµÈ´ýÑ¡Ôñ±»Ëø¶¨µÄµÀ¾ß¡£
char _lock_pos_ = NULL;
long _lock_item_id_ = 0;
long _lock_grid_id_ = 0;
long _lock_fusion_item_id_ = 0;

//extern	bool	g_yyy_add_lock_item_wait_return_state;		//	ÒÑ¾­·¢ËÍËø¶¨ÏûÏ¢£¬µÈ´ý·þÎñÆ÷·µ»ØËø¶¨½á¹û×´Ì¬¡£
...
bool CItemCommand::MouseDown() {
	...
	//	Èç¹ûÊÇµÈ´ýËø¶¨ÏûÏ¢·µ»Ø×´Ì¬£¬ÔòÖ±½ÓÌø¹ý¡£
	...
	//	Èç¹ûÊÇµÈ´ý±»Ëø¶¨×´Ì¬¡£
		...
		if (pOwn) {
			if (pOwn == g_stUIEquip.GetGoodsGrid()) {
				...
				// add by ning.yan 2008-11-10 Èç¹ûµÀ¾ßÒÑËø¶¨£¬Ôò½âËø begin
				if (_ItemData.dwDBID) {
				...
				} else // end
				{
					...
					//delete by ning.yan Ê¹¿òÊ¼ÖÕÏÔÊ¾ÔÚÓÎÏ·½çÃæÖÐÑë 2008-11-17 begin
	...
	if (CRepairState* pState = dynamic_cast<CRepairState*>(pCha->GetActor()->GetCurState())) {
		...
		// ¼Àìë×´Ì¬  add by Philip.Wu  2006-06-20
		/*
...
bool CItemCommand::UseCommand(bool isRightClick) {
	...
	if (pOwn) {
		...
		if (_pItem->lID == 15046 || _pItem->lID == 15045 || _pItem->sType == 66 || (864 <= _pItem->lID && _pItem->lID <= 866)) // Ìí¼Ó¼Àìë×´Ì¬  add by Philip.Wu  2006-06-20
		{
			CActor* pActor = CGameScene::GetMainCha()->GetActor();
			...
			pActor->SwitchState(pState);
			return false;
		} else if (GetItemInfo()->sType == 71) // ¼¼ÄÜµÀ¾ßÊ¹ÓÃ
		{
			CCharacter* pCha = CGameScene::GetMainCha();
			if (!pCha)
				return false;
			...
			if (nCoolDownTime > 0) // ÓÐ cooldown Ê±¼ä
			{
				DWORD nCurTickCount = g_pGameApp->GetCurTick() - 500; // 500ºÁÃëÑÓÊ±´¦Àí
				map<int, DWORD>::iterator it = _mapCoolDown.find(nSkillID);

				if (it != _mapCoolDown.end() && it->second + nCoolDownTime >= nCurTickCount) {
					// cooldown ÖÐ
					g_pGameApp->SysInfo(g_oLangRec.GetString(898), (it->second + nCoolDownTime - nCurTickCount) / 1000 + 1); //"µÀ¾ßÀäÈ·ÖÐ£¬Ê£Óà %d Ãë"
					return false;
				}

				_mapCoolDown[nSkillID] = g_pGameApp->GetCurTick(); // ±£´æÏÂµÀ¾ß¼¼ÄÜÊ¹ÓÃÊ±¼ä
			}

			// ÎÞÐèµã»÷¼¼ÄÜÊÍ·Å¶ÔÏó»ò·¶Î§
		...
		if ((_pItem->sType < 31 || _ItemData.sID == 5939) && pCha == CGameScene::GetMainCha() && g_stUIBoat.GetHuman() == pCha) {
			//	yyy	add	begin!
			//	Èç¹ûÊ¹ÓÃËøµÀ¾ß£¬ÔòÉèÖÃÑ¡Ôñ±»Ëø¶¨µÀ¾ß×´Ì¬¡£¡£
...
void CItemCommand::_ShowWork(CItemRecord* pItem, SGameAttr* pAttr) {
	...
	//else
	//{
	//	PushHint( "Ö°ÒµÏÞÖÆ:ÎÞ", GENERIC_COLOR );
	//}
}

void CItemCommand::_ShowFusionWork(CItemRecord* pAppearItem, CItemRecord* pEquipItem, SGameAttr* pAttr) // ÓÃÓÚÏÔÊ¾ÈÛºÏºóµÀ¾ßµÄÖ°ÒµÏÞÖÆ
{
	bool isFind = false;
	int iAppearIndex = -1;
	int iEquipIndex = -1;
...
void CItemCommand::_ShowWork(xShipInfo* pInfo, SGameAttr* pAttr) {
	...
	for (int i = 0; i < MAX_JOB_TYPE; i++) {
		...
		/*	ÐÞ¸ÄËûÈË´úÂëÇë¼Ó×¢ÊÍ£¬±©·¢»§ÎÊÌâÒÑ¾­ÐÞ¸Ä£¬ºóÀ´ÓÖ³öÏÖ£¬´úÂë±»ÈË¸Ä¶¯£¬×Ö·û´®×ÊÔ´Ò²Ã»ÕÒµ½¡£*/
		if (!isFind) {
...
void CItemCommand::_ShowBody(CItemRecord* _pItem2) {
	...
	for (int i = 1; i < 5; i++) {
		...
		switch (i) {
		case 1:
			str << g_oLangRec.GetString(681);
			break;
		case 2:
			str << g_oLangRec.GetString(682);
			break;
		case 3:
			str << g_oLangRec.GetString(683);
			break;
		case 4:
			str << g_oLangRec.GetString(684);
			break;
		}
	}
	str << '\0';
...
void SItemHint::Convert(SItemGrid& ItemGrid, CItemRecord* pInfo) {
	...
	// ¶ÁÈ¡ÊôÐÔ£¬Èç¹ûÍøÂçÓÐÊôÐÔ£¬Ê¹ÓÃÍøÂçÊôÐÔ£¬·ñÔò´Ó±í¸ñÖÐ¶ÁÈ¡
	int nAttr = 0;
	for (int i = 0; i < defITEM_INSTANCE_ATTR_NUM; i++) {
...
SItemForge& SItemForge::Convert(DWORD v, int nItemID) {
	...
	forge.IsForge = true;
	forge.nHoleNum = v / 1000000000; // ²ÛÊý

	int nStoneData;
	CStoneInfo* pStoneInfo = NULL;
	for (int i = 0; i < 3; ++i) {
		nStoneData = (v / (int)(pow(1000, 2 - i))) % 1000; // ÈýÎ»Ò»È¡
