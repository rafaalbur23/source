...
void CChannelSwitchForm::EventDiscordCheckChange(CGuiData *pSender)
{
	...
	if (bCheck){//off->on
		pCozeForm->OnSystemMsg("Opened Kill Messages Channel");
		pList->SetPos(pList->GetLeft(),  pCozeForm->m_lstDiscordPage->GetTop()- pList->GetHeight());
	}else{//on->off
		pCozeForm->OnSystemMsg("Closed Kill Messages Channel");
...
void CCozeForm::OnDiscordMsg(string strName, string strMsg,DWORD dwColour){	
	...
	//dwColour = 0xFFff0000;
	CCharMsg::SetChannelColor(CCharMsg::CHANNEL_DISCORD, dwColour);
...
void CCozeForm::OnSystemMsg(string strMsg)
{
	if (GuildSysInfo)
		...
		CCharMsg::SetChannelColor(CCharMsg::CHANNEL_SYSTEM, 0xFFFFFF00);//0xFFFFFF00);//Zizo
...
//					pItem->AddHighlightText(0, nPos+1,0xFFFFFF00);
					pItem->AddHighlightText(0, nPos+1,0xFFFF0000);//Zizo
	...
	while (m_cSystemMsg.MoveToNextMsg())
	{
		...
		while (!strMsg.empty())
		{
			...
			if (!bIndentFlag)
			{
				...
				pItem->SetHasHeadText(8, 0xFFFF0000);//Zizo
			}
	...
	if (pThis->m_grdFacePanel->GetSelect()) {
		...
		if (IsNitroOn && !pChar->GetPlayerVip()) {
			if (index >= NitroIndexStart) {
				g_pGameApp->ShowMidText("Selected emoji avilable for VIP1+ players! please support the game with a donation!");
				return;
			}
		}
		//
