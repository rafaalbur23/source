...
{
	// 屏蔽 CTRL + ALT + SHIFT + P 显示的内容

	static char szBuf[128] = { 0 };
	sprintf( szBuf, g_oLangRec.GetString(405), LastPingShow[0], LastPingShow[1], LastPingShow[2], MaxPing, fAveragePing, MinPing,CGameApp::GetFrameFPS() );
	g_pGameApp->GetFont()->DrawText( szBuf, 7, 55, D3DCOLOR_ARGB(255,000,255,000));	//Green color
}
...
//    {
 ...
 //   }
    ...
    // Compare two pathfindings, whether the end points are the same and the end points are different.
    if( isServer )
    {
    ...
    // The first point is your own coordinates
...
// Take note of the new movement path
bool COneMoveState::ContinueMove( int nTargetX, int nTargetY, bool isWalkLine, bool IsCheckTime )
{	
	// Check whether the coordinates are legal and illegal, and pull back directly
	...
	// Cache user requested movement paths
	_cNeedList.clear();
	while( !_List.IsEmpty() )
	{
...
//	}
	...
	// Adjust the protagonist rate, and if the distance is too far, pull it back directly
	if( abs( nLocal - _nServerDis ) > GetSyschroDistance() )
	{
		// Distance is too large, pull back
		_fMoveRate = _fNormalRate;
		_fRate = _fNormalRate;
		SynchroPos( _pCha->GetServerX(), _pCha->GetServerY() );
...
{
	//The distance that needs to be synchronized, that is, when the distance is exceeded, you must directly pull back to the protagonist
...
{
	//Dynamically adjust the movement speed of the protagonist
	...
	{
		// The server has returned to the end of the move, checks that it is a cache move, and sends a new move protocol
		if( !SendInfo() )
		{
			PopState();
			return;
	...
	if( _pCha->GetIsArrive() )
	{
		...
		{
			// The server has returned to the end of the move, and there is no cache move, and the status is ended
			PopState();
			return;
		}
		...
		{			
			// The protagonist has moved to the destination before the server and is waiting for the server's end sign
			// In order to prevent people from walking in place, call Idle directly
			GetActor()->IdleState();
		}
	}
	...
	if( _nSendCount==_nEndCount && CGameApp::GetCurTick()>_dwEndTime )
	{
		...
		{
			// Check if the next status is attack status and the server has returned to end
			// Regardless of whether there is a cache move, end this move state and prepare to attack
			PopState();
			return;
		}
		...
		if( !_cNeedList.empty() )
		{
			// The server has returned to the end, performing a new cache move
...
{
	// There is a new movement path, sending a protocol to terminate the current movement, and the protagonist walks along the new path in advance
	if( !IsAllowMove() )
		return false;
...
//		return false;
	...
	// Adjust the moving speed of the protagonist according to the distance ratio between the local and server
//	if( g_cFindPath.GetPathFindingState() )
...
{
	// Pull the lead directly to the new coordinates
	_pCha->setPos( x, y );
	_pCha->MoveTo( x, y );
...
void COneMoveState::MoveEnd(int x, int y, int nState)
{
	...
	// If you encounter an obstacle and stop, stop directly
