...
void CWorldScene::_FrameMove( DWORD dwTimeParam )
{
	...
	case GUI::enumMA_Skill:
		...
		else
		{
			...
			if( pSkill && pSkill->IsAttackArea() &&!CGameScene::GetMainCha()->GetIsOnMount() )
			{
				// Change the mouse to attack the ground
			...
			else
			{
				...
				// // Single skill, detecting whether the attack object is correct
				if( pSkill )
				{
					CCursor::I()->SetFrame( CCursor::stSkillAttack );
				}
	...
	case GUI::enumMA_None:
		...
		{	
			static bool g_bIsMouseWalk = false;
			...
			{
				// Change the mouse to attack single player
				g_pGameApp->GetCursor()->SetCursor( csNormalCursor );
				CGameScene::Set3DMouseState( enumClick );
				CCursor::I()->Restore();
				...
				if( !IsShop )
				{
					pCha = HitSelectCharacter( nMouseX, nMouseY); //MobInfo enumSC_PLAYER
					if( pCha )
					{
...
bool CWorldScene::_HandleSuperKey()
{
    ...
    else if(g_pGameApp->IsKeyDown(DIK_5))
    {
        ...
        g_pGameApp->ChangeVideoStyle(SMALL_RES_X, SMALL_RES_Y, D3DFMT_D16, be_windowed);
    }
...
BOOL CWorldScene::_InitUI()
{
	...
	form = CFormMgr::s_Mgr.Find( "frmFast2" ); // BARAN MOD
	if( form )
	{
		form->Show();
	}
...
void CWorldScene::_SceneCursor()
{
    ...
    if( g_pGameApp->IsShiftPress() )
    {
        ...
        for(int i = 0; i < _nChaCnt; i++)
        {
            if( pCha->IsValid() && !pCha->GetBlockTargetCha()) 
            {
                if( pCha->IsEnabled() )
                {
	...
	if( pSkill && pSkill->IsAttackArea() &&!pMain->GetIsOnMount())
	{
		// 改变鼠标为攻击地面
		CGameScene::Set3DMouseState( enumFollow );
		g_pGameApp->GetCursor()->SetCursor( csCircleCursor );
	...
	else
	{
		...
		// µ¥Ìå¼¼ÄÜ,¼ì²â¹¥»÷¶ÔÏóÊÇ·ñÕýÈ·
		if( pSkill && !pMain->GetIsOnMount() )
		{
			CCursor::I()->SetFrame( CCursor::stSkillAttack );
			...
			if( g_pGameApp->IsCtrlPress() )
			{
				...
				if( pAttackCha && !pMain->GetIsOnMount() )
				{
			...
			else
			{
				...
				if( pAttackCha ) // if( pAttackCha && !pMain->GetIsOnMount() )
				{
					pAttackCha->GetHeadSay()->SetIsShowName( true );
		...
		else
		{
			...
			{
				pAttackCha->GetHeadSay()->SetIsShowName( true );
				if (g_pGameApp->IsCtrlPress() && pMain != pAttackCha && !pMain->GetIsOnMount() )
				{
				...
				else if( (pSkill=pMain->GetDefaultSkillInfo()) && g_SkillUse.IsAttack( pSkill, pMain, pAttackCha ) && !pMain->GetIsOnMount() && pAttackCha->getChaCtrlType() != enumCHACTRL_Mount)
				{
					CCursor::I()->SetFrame( CCursor::stAttack ); 
				...
				else if( pAttackCha!=pMain && pAttackCha->IsPlayer() && !pMain->GetIsOnMount() )
				{
