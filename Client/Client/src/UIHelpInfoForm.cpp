...
void CHelpInfoMgr::ShowHelpInfo( bool show, const char* HelpTitle )
{
	if( show )
	{
		...
		if( HelpInfo )
			...
			sprintf( page.szDesp, GetHelpInfo( HelpTitle ), ROLE_MAXNUM_DESPSIZE - 1);
...
void CHelpInfoMgr::_ItemClickEvent( string strItem )
{
	...
	if(bmap && bx && by && index == strItem.length())
	{
		...
		if(strcmp((const char*)map,(const char*) g_oLangRec.GetString(56) ) == 0)
		{
			targetmap = new char[9];
			targetmap = g_oLangRec.GetString(56);
		}
		else if(strcmp((const char*)map,(const char*) g_oLangRec.GetString(57) ) == 0)
		{
			targetmap = new char[9];
			targetmap = g_oLangRec.GetString(57);
		}
		else if(strcmp((const char*)map,(const char*) g_oLangRec.GetString(58) ) == 0)
		{
			targetmap = new char[9];
