...
void	CAniClock::Render(int x,int y)
{
	...
	minutes > 0 ? _snprintf(buf, 6, "%.2d:%.2d", minutes, seconds)
				: seconds > 0 ? _snprintf(buf, 6, "%d", seconds)
							  : _snprintf(buf, 6, "%.1f", seconds_remaining);
	CGuiFont::s_Font.BRender(0, buf, x + 1, y + 20, COLOR_WHITE, COLOR_BLACK);
}
//////////////////////////////////////////////////////////////////////////
...
void	CCharacter2D::LoadCha(DWORD dwID, bool IsMobInfo){
	if(!_pModel){
...
void CCharacter2D::UpdataFace(stNetTeamChaPart& stPart, bool IsPlayer)
{
	...
	int nHairID;
	if (stPart.SLink[enumEQUIP_HEADAPP].sID!=0 && CCharacter::GetIsShowApparel()) nHairID = stPart.SLink[enumEQUIP_HEADAPP].sID;
   	else nHairID = stPart.SLink[enumEQUIP_HEAD].sID!=0 ? stPart.SLink[enumEQUIP_HEAD].sID : stPart.sHairID;
	int nSelfID;
	if(_pChaPart->SLink[enumEQUIP_HEADAPP].sID!=0 && CCharacter::GetIsShowApparel()) nSelfID = _pChaPart->SLink[enumEQUIP_HEADAPP].sID;
	else nSelfID = _pChaPart->SLink[enumEQUIP_HEAD].sID!=0 ? _pChaPart->SLink[enumEQUIP_HEAD].sID : _pChaPart->sHairID;
