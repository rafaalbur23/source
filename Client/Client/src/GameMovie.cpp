...
bool CGameMovie::Play(HWND hWnd, TCHAR* pszFileName)
{
	...
	*/
	...
	int width = (rc.bottom - rc.top) * 16.0 / 9.0;
	int x = ((rc.right - rc.left) - width) / 2.0;
	_pVidWin->SetWindowPosition(x, rc.top, width, rc.bottom - rc.top);
	...
	Stop();
	_hWnd = 0;
	//_hResult = _pVidWin->put_Owner((OAHWND)NULL);
