...
namespace GUI
{
	...
	bool CStoreMgr::Init()
	{
		...
		*/
		...
		// Evg add: Enter Redeem code
		CForm* frmRedeem = _FindForm("frmRedeem");
		frmRedeem->evtEntrustMouseEvent = _evtRedeemFormMouseEvent;
...
/*
	...
	// Redeem code form 
	void CStoreMgr::_evtRedeemFormMouseEvent(CCompent *pSender, int nMsgType, int x, int y, DWORD dwKey)
	{
		string strName = pSender->GetName();
		
		CForm* frmRedeem = _FindForm("frmRedeem");
		CEdit* edtKey = dynamic_cast<CEdit*>(frmRedeem->Find( "edtKey" ));
		if(strName == "btnConfirm")
		{
			CS_StoreBuyCrystal(edtKey->GetCaption());
			frmRedeem->Close();
		}
	}
	...
	void CStoreMgr::_evtStoreFormMouseEvent(CCompent *pSender, int nMsgType, int x, int y, DWORD dwKey)
	{
		...
		else if(strName == "btnRedeem")
        {			
            CForm* frmRedeem = _FindForm("frmRedeem");
		    frmRedeem->SetIsShow(!frmRedeem->GetIsShow());
