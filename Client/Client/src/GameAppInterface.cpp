...
#include "packdata/TerrainInfo.h"
#include "Mountinfo.h"
...
bool CGameApp::isRegFlag(){
	return regFlag;
}

bool CGameApp::isLoadFlag(){
	return loadFlag;
}

void CGameApp::setRegFlag(bool flag){
	regFlag = flag;
}
...
void CGameApp::_ShowLoading(int percent)
{
    ...
    if( frmLoading )
    {
		...
		if(frmLoading)
		{
			CImage* imgLoadingBar2 = dynamic_cast<CImage*>(frmLoading->Find("imgLoadingBar2"));
			if(imgLoadingBar2)
			{
				int wX = (int)(w_loadingbar - w_loadingbar * percent / 100);
				imgLoadingBar2->SetPos((int)(GetRender().GetScreenWidth() / 5) + 7, (int)(GetRender().GetScreenHeight()  * 0.935f) + 12);
				imgLoadingBar2->SetSize(wX, 2);
				imgLoadingBar2->Refresh();
			}
		}
...
void CGameApp::RefreshLoadingProgress()
{
	...
	if(frmLoading)
	{
		...
		CImage* imgLoadingBar = dynamic_cast<CImage*>(frmLoading->Find("imgLoadingBar2"));
		if(imgLoadingBar)
		{
			float fPrecent = ((GetCurTick() - _dwLoadingTick) % PROGRESS_CRYCLE) / (float)(PROGRESS_CRYCLE);	// 在当前进度条的百分比
			int wX = (int)(1180 * fPrecent);
...
void CGameApp::ResetCaption()
{
	...
	char szCaption[350]; sprintf(szCaption, CLIENT_NAME, (float)(g_sClientVer % 1000) / 100, szSpace ); // , curr_ver.c_str());
...
void CGameApp::Loading( int nFrame )
{
	...
	string path = "texture/ui/LOADING/";
	string szLoadingPic;
	vector<string> lines;
	...
	if(!((hFind = FindFirstFile("texture/ui/LOADING/*", &fileData)) == INVALID_HANDLE_VALUE))
	{
	...
	int random_number = rand() % files.size();
	szLoadingPic = files[random_number];
	if (!frmLoading )
	{
	...
	else if (GetRender().GetScreenWidth()==MID_RES_X)
	{
		width = MID_RES_X/4;
	}
	...
	else if (GetRender().GetScreenWidth()==EXTRA_LARGE_RES_X)
	{
		width = EXTRA_LARGE_RES_X/4;
	}
	else if (GetRender().GetScreenWidth()==FULL_LARGE_RES_X)
	{
		width = FULL_LARGE_RES_X/4;
	}
	else
	{
		width = SMALL_RES_X/4;
	...
	CImage  *imgLoadingbg = NULL;
	imgLoadingbg = dynamic_cast<CImage *>(frmLoading->Find("imgLoadingbg"));
	//imgLoadingbg->SetPos(131, 0);
	imgLoadingbg->_pImage->LoadImage(szLoadingPic.c_str());
	imgLoadingbg->SetSize( GetRender().GetScreenWidth()  ,GetRender().GetScreenHeight());
	imgLoadingbg->Refresh();

	CImage* imgLoadingBar1 = dynamic_cast<CImage*>(frmLoading->Find("imgLoadingBar1"));
	if(imgLoadingBar1)
	{
		imgLoadingBar1->SetPos((int)(GetRender().GetScreenWidth() / 5), (int)(GetRender().GetScreenHeight() * 0.935f));
		imgLoadingBar1->SetSize((int)(GetRender().GetScreenWidth() / 5 * 3), 20);
		imgLoadingBar1->Refresh();
	}

	//CImage  *imgLoading = NULL;
	// CImage  *imgLoadingBar = NULL;
    // char szBuf[8];
    ...
    //     imgLoadingBar = dynamic_cast<CImage *>(frmLoading->Find("imgLoadingBar2"));
    //     if (imgLoadingBar)
    //     {
    //         imgLoadingBar->SetSize( i*75 , 3);
    //         imgLoadingBar->Refresh();
    //     }
    // }
}
...
void CGameApp::InitAllTable()
{
	...
	MountHelper* MountHelperSet = new MountHelper(0, 1000);
...
void CGameApp::ReleaseAllTable()
{
	...
	CMonsterSet* pMonsterSet = CMonsterSet::I();
	SAFE_DELETE( pMonsterSet );
	...
	MountHelper* pMountHelperSet = MountHelper::I();
	SAFE_DELETE(pMountHelperSet);
}
...
void CGameApp::ShowVideo()
{
	HWND handle = g_pGameApp->GetHWND();
	//m_pMovie = new CGameMovie();
	//m_pMovie->Play(handle, "first.mkv");
