...
inline void SetPreName( int nItem, char* szName, DWORD& dwColor )
{
	...
	DWORD newneckcolor = D3DCOLOR_ARGB(255, 84, 233, 123);
	...
	case 15947:	strcpy( szName, "Officer" ); dwColor = newneckcolor; return;
...
void CCharacter::_UpdateHeight()
{
    ...
    if( _nPoseHeightOff==ERROR_POSE_HEIGHT )
    {
		...
		if(GetIsOnMount()) {
			...
			MPMatrix44 mat;
			pMount->GetRunTimeMatrix(&mat, pMountBone); // Hello Mothanna, i'm yagura bro!

			_vPos.z = mat._43 + (MountHeight / 100.0f);
...
bool CCharacter::PlayPose( DWORD pose, DWORD type, int time, int fps, bool isBlend )
{
	...
	if (GetIsOnMount() && !IsHide()) {
		...
		if (pMount)
		{
			...
			else if (pMount->GetCurPoseType() == POSE_DIE || pMount->GetCurPoseType() == POSE_FALLDOWN)
			{
				pMount->SetHide(TRUE);
				pose = POSE_DIE;
			}
			else {
				pMount->SetHide(FALSE);
			}
	...
	if( isBlend )
	{
		CCharacterModel::PlayPose(pose, type, time, fps, velBoost);	
	}
	else
	{
...
bool CCharacter::UpdataItem( int nItem, DWORD nLink )
{
	...
	switch(nLink){
		...
		if (dynamic_cast<CWorldScene*>(_pScene) && this->IsPlayer())
		{
			MountFunction(nItem);
		}
	...
	case enumEQUIP_GLOWAPP:    //fixing glow problem //
...
void CCharacter::SetHide(BOOL bHide)
{
	...
	if (pMount) pMount->SetHide(bHide);	
...
bool CCharacter::GetIsOnMount(){
	...
	if(GetIsPK()) return false;
	return static_cast<bool>(pMount);
}

bool CCharacter::GetCanPkOnMount() {
	...
	if (pMount)
	{
		if (GetIsPK())
		{
			if (!pMount->IsHide())
			{
				pMount->SetHide(TRUE);
				if(IsPlayer())
					GetScene()->GetMainCha()->PlayPose(POSE_WAITING2, PLAY_LOOP_SMOOTH);	//
...
void CCharacter::_computeLinkedMatrix()
{
	...
	{
		MPMatrix44 mat;
		pMount->GetRunTimeMatrix(&mat, pMountBone);
		if (pMount->GetCurPoseType() == POSE_DIE || pMount->GetCurPoseType() == POSE_FALLDOWN) {

			setHeightOff(GetDefaultChaInfo()->fHeight);	//check if player died or not @mothannakh

		}
		else if (GetCanPkOnMount()) {

			setHeightOff(GetDefaultChaInfo()->fHeight);

		}
		else  setHeightOff(int((mat._43 * AutoMountHeight - this->MountHeight))); //to fix auto height for some mounts, 
		
		if (_pNpcStateItem && !GetCanPkOnMount())
		{

			int NpcHeight = (int)(GetDefaultChaInfo()->fHeight * (int((mat._43 * AutoMountHeight - this->MountHeight))));
			if (NpcHeight > 600) NpcHeight = 500;
			_pNpcStateItem->setHeightOff(NpcHeight);
		}
	...
	if (mParentNode)
	{
		...
		//conver mount scale from mountinfo.txt
		scale.x = mParentNode->MountScale;
		scale.y = mParentNode->MountScale;
		scale.z = mParentNode->MountScale;
		SetScale(scale);
		float dx = -mParentNode->MountY * cos(rotZ) - mParentNode->MountX * sin(rotZ);
		float dy = -mParentNode->MountY * sin(rotZ) + mParentNode->MountX * cos(rotZ);
		setPos(int(pos.x * 100.f + dx), int(pos.y * 100.f + dy));
		setYaw(int(yaw));
	}
...
void CCharacter::MountFunction(int nItem)
{
	int ID = nItem;
	if (pMount && ID != pMountID) {
		RemoveMount();
	}
	if (ID > 0 && !GetIsPK() && this->IsPlayer() && g_stUISystem.m_sysProp.m_gameOption.bIsShowMount) {
		if (!pMount) {
			MountInfo* MountInfo = GetMountInfoFromItemID(ID);
			if (!MountInfo) return;
			int mountid = MountInfo->CharID;
			int chatype = (this->GetDefaultChaInfo()->lID);
			this->pMountBone = MountInfo->mountBone[chatype - 1];
			//check if mount valid id or not 
			CChaRecord* MountInfoX = GetChaRecordInfo(mountid);
			
			if (!MountInfoX) mountid = 1;	//if not then we use lance as mount to fix it 	
			pMount = _pScene->AddCharacter(mountid);
			pMount->SetBlockTargetCha(true);
			pMount->setName("");
			//mount pose depends on model 
			this->MountPose = MountInfo->PoseID[chatype - 1];
			//check if mount has custom height or not 
			this->MountHeight = MountInfo->MountHeight[chatype - 1];
			//Custom height end 
			//mount scale 
			...
			pMount->linkTo(this, pMountBone);	//1
			pMount->setChaCtrlType(enumCHACTRL_Mount);
			pMount->setMobID(mountid);
			pMount->_Attr.set(ATTR_LV, 10);
			this->pMountID = ID;
			pMount->SetHide(IsHide());
