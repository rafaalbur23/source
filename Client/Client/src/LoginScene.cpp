...
void CLoginScene::_Render() {
	...
	static CGuiPic LoginPic;
	if (!IsLoad) {
		LoginPic.LoadImage("texture/ui/new_login.jpg", FULL_LARGE_RES_X, FULL_LARGE_RES_Y, 0, 0, 0, 1.F, 1.F);
		IsLoad = true;
	}
...
void CLoginScene::_evtLoginFrm(CCompent *pSender, int state, int x, int y, DWORD key)
{
	...
	{
		if(g_pGameApp->isRegFlag()){
			g_pGameApp->MsgBox("You have to restart game for register again");
			return;
		} 
	...
	else if(strName == "btnAlpha"){
		CEdit* edtRegEmail = dynamic_cast<CEdit*>(frmRegister->Find( "edtRegEmail" ));
		string str = edtRegEmail->GetCaption();
		str += "@";
		edtRegEmail->SetCaption(str.c_str());
	}
}
...
bool CLoginScene::_CheckAccount()
{
	...
	if(registerLogin){
		...
		if(strlen(edtRegPassword->GetCaption()) < 4){
			g_pGameApp->MsgBox("Password cannot be less than 4 letters!");
			return false;
		}
