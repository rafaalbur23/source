...
BOOL SC_StoreCrystal(LPRPACKET pk)
{T_B
	int crystal = pk.ReadLong();	// bought crystal
	if(crystal == -1)
	{
		g_pGameApp->MsgBox("Invalid Key");
	}
	else 
	{
		int total = pk.ReadLong();
		int bean = pk.ReadLong();
		char str[1024];
		sprintf(str, "You bought %d crystals successfully.\nYour total crystal number is %d.", crystal, total);
		g_pGameApp->MsgBox(str);
		g_stUIStore.SetStoreMoney(bean, total);
	}
