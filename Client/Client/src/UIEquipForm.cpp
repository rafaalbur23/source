...
bool CEquipMgr::Init()
{
	...
	SetIsLock(false);	// Not locked by default
	...
	CForm* frmFast2 = _FindForm("frmFast2"); // BARAN MODE
	...
	for( DWORD j=0; j<SHORT_CUT_NUM; j++ )
    {
		...
		}else if (j >= MAX_FAST_COL*2) // BARAN MODE
		{
			sprintf( szName, "fscMainN%d", j -12);
...
void CEquipMgr::evtThrowItemEvent(CGuiData *pSender,int id,bool& isThrow)
{
    ...
    else if( obj->GetIsPile() && obj->GetTotalNum()>1 )
    {
	...
	else if ( pItem && (pItem->GetItemInfo()->lPrice > 0 || pItem->GetItemInfo()->sNeedLv > 0 ) )
	{
		
		char buf[256] = { 0 };
		sprintf(buf, g_oLangRec.GetString(959), pItem->GetItemInfo()->szName);
		stSelectBox* pBox = g_stUIBox.ShowSelectBox(_evtThrowBoatDialogEvent, buf, true);
