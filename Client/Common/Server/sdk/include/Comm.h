...
class TcpClientApp:public TcpCommApp
{
	friend class TcpServerApp;
public:
	TcpClientApp(RPCMGR *rpc,ThreadPool *processor,ThreadPool *communicator,bool mode =true, bool SSL = false);//mode =false 强制“数据包”不进入处理线程池
	~TcpClientApp(){};
...
{
public:
	TcpServerApp(RPCMGR *rpc,ThreadPool *processor,ThreadPool *communicator,bool mode =true, bool SSL = false);//mode =false 强制“数据”包不进入处理线程池
	~TcpServerApp();
