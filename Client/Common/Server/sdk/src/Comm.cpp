...
TcpCommApp::TcpCommApp(RPCMGR *rpc,ThreadPool *processor,ThreadPool *communicator,bool mode)
		...
		,__atnotconn(0),__conntotal(0),__deltotal(0),__bSSL(false)
{
...
//==============TcpClientApp==================================================================
TcpClientApp::TcpClientApp(RPCMGR *rpc,ThreadPool *processor,ThreadPool *communicator,bool mode, bool SSL)
		:TcpCommApp(rpc,processor,communicator,mode)
{
		...
		if(__bSSL)
		{
			...
			//load certificate
			  SSL_CTX_load_verify_locations( client_ctx, "system/ca.crt", NULL );
...
DataSocket *	TcpClientApp::Connect(cChar *hostname,uShort port,SOCKET *sock_out)
{
	if(!__atnotconn)
	{
		...
		if(__bSSL)
		{
			ssl = SSL_new(client_ctx); /* get new SSL state with context */
			SSL_set_fd(ssl, l_sock);
			//~ssl
		...
		l_datasock->Init(l_sock, ssl, inet_ntoa(l_peersaddr.sin_addr),port,this,false);
...
//==============TcpServerApp==================================================================
TcpServerApp::TcpServerApp(RPCMGR *rpc,ThreadPool *processor,ThreadPool *communicator,bool mode, bool SSL)
		:__socket(INVALID_SOCKET),__port(443),__acceptflag(false)
...
// initialize server context
	if(__bSSL)
	{
		...
		//printf("use certificate file\n");

		if ( SSL_CTX_use_certificate_file(server_ctx, "server.crt", SSL_FILETYPE_PEM) != 1 )
		{
			ERR_print_errors_fp(stderr);
			;
		}
		//printf("use private file\n");
		/* set the private key from KeyFile (may be the same as CertFile) */
		if ( SSL_CTX_use_PrivateKey_file(server_ctx, "server.key", SSL_FILETYPE_PEM) != 1 )
		{
			ERR_print_errors_fp(stderr);
			;
		}
		...
		//SSL_CTX_set_client_hello_cb(server_ctx, client_hello_cb, NULL);
	}
	//~ssl
}
