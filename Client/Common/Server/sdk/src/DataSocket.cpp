...
,m_rpcinfo(0)
,m_sender(*new Sender(this)),m_receiver(*new Receiver(this)), m_ssl(0)
{
	Initially();
}
...
void DataSocket::Init(SOCKET socket, SSL *ssl, cChar *peerip,uShort peerport,TcpCommApp *tca,bool IsServer)
{
	*const_cast<SOCKET*>(&m_socket)	=socket;
	const_cast<SSL*>(m_ssl) = ssl;
