...
public:
	...
	bool		CanStack(SItemGrid* item1, SItemGrid* item2);	
...
inline short CKitbag::Push(SItemGrid *pGrid, short &sPosID, short sType, bool bCommit, bool bSureOpr)
{
	...
	if (sPosID >= 0 && sPosID < m_sCapacity) // 指定位置
	{
		if (bCommit)
		{
			...
			if (CanStack(&m_SItem[sType][sPosID].SContent,pGrid))				
			{
	...
	if (sPileNum > 1) // 可以合并，则搜索相同道具的栏位
	{
		for (short i = 0; i < m_sCapacity; i++)
		{
			...
			if (CanStack(&m_SItem[sType][i].SContent, pGrid))
			{
...
/*
	new check for stacking items - items must be identical.
	This means we can now stack gems of the same level.
	
	TODO:
		May interfere with locked items.
*/
inline bool CKitbag::CanStack(SItemGrid* item1, SItemGrid* item2){
	if (
		(item1->sID != item2->sID)
		|| (item1->sEndure[0] != item2->sEndure[0] || item1->sEndure[1] != item2->sEndure[1])
		|| (item1->sEnergy[0] != item2->sEnergy[0] || item1->sEnergy[1] != item2->sEnergy[1])
		|| (item1->lDBParam[0] != item2->lDBParam[0] || item1->lDBParam[1] != item2->lDBParam[1])
		|| (item1->GetForgeLv() != item2->GetForgeLv())
		|| ((item1->dwDBID>0) != (item2->dwDBID>0))
		|| (item1->IsInstAttrValid() != item2->IsInstAttrValid())
	){
		return false;
	}
	if (item1->IsInstAttrValid()){
		for (int i = 0; i < defITEM_INSTANCE_ATTR_NUM; i++){
			if (item1->sInstAttr[i][0] != item2->sInstAttr[i][0] || item1->sInstAttr[i][1] != item2->sInstAttr[i][1]){
				return false;
			}
		}
	}
...
inline short CKitbag::Regroup(short sSrcPosID, short sSrcNum, short sTarPosID, short sType)
{
	...
	if (CanStack(pSSrcCont,pSTarCont)) // 相同道具
	{
		CItemRecord *pCItem;
...
inline bool String2KitbagData(CKitbag *pKitbag, std::string &strData)
{
	...
	__int64	lnCheckSum	= 0;
	const short csStrNum = 1 + enumKBITEM_TYPE_NUM * defMAX_KBITEM_NUM_PER_TYPE * 2 + 1 + 1 + 1;
